...
export const Route = createFileRoute("/analytics/participation")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Participation funnel, regional reach and category mix insights." },
      { property: "og:title", content: "Participation analytics · Ascent Platform" },
      { property: "og:description", content: "Participation funnel, regional reach and category mix insights." },
    ],
  }),
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <SectionCard title="Participation trend" description="Registrations vs participants">
          <TrendAreaChart data={registrationTrend} xKey="month" series={[{ key: "registrations", label: "Registrations" }, { key: "participants", label: "Participants" }]} height={240} />
        </SectionCard>
        ...
        <SectionCard title="By region" description="Participants and teams">
          <GroupedBarChart data={participationByRegion} xKey="region" series={[{ key: "participants", label: "Participants" }, { key: "teams", label: "Teams" }]} height={240} />
        </SectionCard>
      </div>
          ...
          {funnel.map((stage) => (
            <div key={stage.stage}>
              <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 text-sm">
                ...
                <span className="shrink-0 tabular-nums text-muted-foreground">{stage.value.toLocaleString()}</span>
              </div>
