...
export const Route = createFileRoute("/events/schedule")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Agenda builder with tracks, speakers and session types for every event day." },
      { property: "og:title", content: "Event schedule · Ascent Platform" },
      { property: "og:description", content: "Agenda builder with tracks, speakers and session types for every event day." },
    ],
  }),
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><Button variant="outline">Export agenda</Button><Button>Add session</Button></>}
      />
      <div className="space-y-6">
        {days.map((day) => (
          <SectionCard key={day} title={day} description={`${scheduleItems.filter((s) => s.day === day).length} sessions`} padded={false}>
            <ul className="divide-y divide-border">
              {scheduleItems.filter((s) => s.day === day).map((item) => (
                <li key={item.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3 px-5 py-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-xs text-muted-foreground">{item.track} · {item.speaker}</p>
                  </div>
