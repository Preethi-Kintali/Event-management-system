...
export const Route = createFileRoute('/principal/proposals')({
  component: PrincipalProposalsComponent,
});

function PrincipalProposalsComponent() {
  const queryClient = useQueryClient();

  const { data: response, isLoading } = useQuery({
    queryKey: ['principal-proposals'],
    queryFn: async () => {
      const res = await fetchApi('/hackathon-proposals/principal/pending');
      return res.data;
    },
  });
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ id, action, comment }: { id: string, action: string, comment?: string }) => {
      ...
      await fetchApi(`/hackathon-proposals/${id}/principal-review`, { 
        method: 'POST', 
        body: JSON.stringify(body) 
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['principal-proposals'] });
  ...
  const filteredProposals = proposals.filter((p: any) => {
    ...
    const isActive = p.status === 'SUBMITTED_TO_PRINCIPAL';
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Principal Approval Queue</h1>
              ...
              <div key={p.id} className="border p-4 rounded bg-card shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-sm text-green-700">Manager Approved by: {p.manager?.firstName} {p.manager?.lastName}</p>
                  </div>
