...
export const Route = createFileRoute("/certificates/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Preview, verify and download issued certificates with serial validation." },
      { property: "og:title", content: "Certificate preview · Ascent Platform" },
      { property: "og:description", content: "Preview, verify and download issued certificates with serial validation." },
    ],
  }),
  ...
  return (
    <>
      <PageHeader
        ...
        crumbs={[{ label: "Evaluation" }, { label: "Certificates", to: "/certificates" }, { label: "Preview" }]}
        actions={<><Button variant="outline"><Printer className="h-4 w-4" />Print</Button><Button><Download className="h-4 w-4" />Download PDF</Button></>}
      />
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <SectionCard title="Preview" description="A4 landscape · 300 dpi">
          <div className="mx-auto flex aspect-[1.414/1] w-full max-w-3xl flex-col items-center justify-center rounded-xl border-2 border-primary/25 bg-surface p-10 text-center shadow-card">
            <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">Certificate of achievement</p>
            <h2 className="text-display mt-6 text-3xl font-semibold">Rhea Kapoor</h2>
            <p className="mt-4 max-w-lg text-sm text-muted-foreground">
              has been awarded first place in the Campus Robotics Sprint at Northwind Hack the Campus,
              held 22–24 August 2026.
            </p>
