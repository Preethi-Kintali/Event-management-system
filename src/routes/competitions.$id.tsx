...
function CompetitionDetailsPage() {
  return (
    <DetailsPageTemplate
      ...
      crumbs={[{ label: "Programs" }, { label: "Competitions", to: "/competitions" }, { label: "Competition details" }]}
      ...
      metrics={[{ label: "Teams", value: "214" }, { label: "Submissions", value: "186" }, { label: "Avg score", value: "76.4" }, { label: "Judges", value: "9" }]}
      related={[{ id: "r1", label: "Neural Nomads", meta: "Team · 4 members" }, { id: "r2", label: "SUB-2291", meta: "Submission · In review" }, { id: "r3", label: "Dr. Elena Marković", meta: "Judge · Machine Learning" }]}
      overview={(
        <>
          <SectionCard title="Evaluation load" description="Pending vs completed per round">
            <GroupedBarChart data={evaluationLoad} xKey="round" series={[{ key: "completed", label: "Completed" }, { key: "pending", label: "Pending" }]} stacked height={260} />
          </SectionCard>
          <SectionCard title="Submission board" description="Drag-free kanban view" padded={false}>
            <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
              {kanbanColumns.map((column) => (
                <div key={column.id} className="space-y-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{column.title} · {column.items.length}</p>
                  {column.items.map((item) => (
                    <KanbanCard key={item.id} title={item.title} subtitle={item.team} meta={item.id} badge={<StatusChip status={item.status} />} />
                  ))}
                </div>
              ))}
            </div>
